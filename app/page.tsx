...
export default function Home(){
 ...
 return <main>
  <header className="hero"><nav className="nav-shell"><div className="brand"><span className="brand-mark">B</span><span>Buonissimo!</span></div><div className="nav-note">Le ricette provengono da Alimentazione Bambini, il sito sviluppato da Coop. Per informazioni sulla redazione scientifica, <a href="https://alimentazionebambini.e-coop.it/redazione/" target="_blank" rel="noreferrer">clicca qui</a>.</div></nav><section className="hero-content"><p className="eyebrow">100 idee · da 1 a 3 anni</p><h1>Oggi cosa<br/><em>prepariamo?</em></h1><p className="hero-copy">Trova una ricetta partendo da quello che hai in frigo, oppure lascia decidere alla magia.</p><p className="hero-credits">Sviluppato da Carlo, Elena e Marco</p><button className="magic-button" onClick={randomRecipe}><span>✦</span> Scegli per me</button></section><div className="hero-orb hero-orb-one"/><div className="hero-orb hero-orb-two"/></header>
  ...
  {selected&&<section className="magic-pick"><div><span className="spark">✦</span><p>La ricetta magica di oggi</p><h2>{selected.title}</h2></div><button onClick={()=>setSelected(null)} aria-label="Chiudi proposta">×</button><a href={`#${selected.source_url.split("/").filter(Boolean).pop()}`}>Scopri la ricetta →</a></section>}
  <section className="content-shell"><div className="result-heading"><div><p className="eyebrow dark">La selezione</p><h2>{filtered.length} ricette da provare</h2></div><p>{ingredient?`Con “${ingredient}”`:"Filtra, scegli e porta in tavola qualcosa di buono."}</p></div>
